...
class SyncLegacyData extends Command
{
     ...
     * @var string
     */
    protected $signature = 'app:sync-legacy-data {--force : Force sync without confirmation}';
     ...
     * @var string
     */
    protected $description = 'Incrementally synchronize latest data from legacy AltumCode database without duplicates';
    ...
    public function handle()
    {
        ...
        try {
            ...
            $this->error('Please verify DB_CONNECTION_LEGACY or database settings in .env');
        ...
        try {
            // 2. Sync Users (UPSERT by user_id)
            $this->syncUsers($legacyDb);

            // 3. Sync Projects (UPSERT by project_id)
            $this->syncProjects($legacyDb);

            // 4. Sync Domains (UPSERT by domain_id)
            $this->syncDomains($legacyDb);

            // 5. Sync Pixels (UPSERT by pixel_id)
            $this->syncPixels($legacyDb);

            // 6. Sync Links (UPSERT by link_id)
            $this->syncLinks($legacyDb);

            // 7. Sync Biolink Blocks (UPSERT by biolink_block_id)
            $this->syncBiolinkBlocks($legacyDb);

            // 8. Sync Track Links (Append new records where id > max_current_id)
            $this->syncTrackLinks($legacyDb);

            // 9. Align Auto Increment sequence
            $this->alignAutoIncrements();

            $this->info('==================================================');
            $this->info('✓ Incremental sync completed with ZERO duplicates!');
            $this->info('==================================================');
        } catch (\Exception $e) {
    ...
    {
        $this->info('Syncing users (UPSERT)...');
        ...
        $this->line("  -> Users: {$created} added, {$updated} updated.");
    }

    private function syncProjects($legacyDb)
    {
        $this->info('Syncing projects (UPSERT)...');
        ...
        $this->line("  -> Projects: {$created} added, {$updated} updated.");
    }

    private function syncDomains($legacyDb)
    {
        $this->info('Syncing domains (UPSERT)...');
        ...
        $this->line("  -> Domains: {$created} added, {$updated} updated.");
    }

    private function syncPixels($legacyDb)
    {
        $this->info('Syncing pixels (UPSERT)...');
        ...
        $this->line("  -> Pixels: {$created} added, {$updated} updated.");
    }

    private function syncLinks($legacyDb)
    {
        $this->info('Syncing links (UPSERT)...');
        ...
        $this->line("  -> Links: {$created} added, {$updated} updated.");
    }

    private function syncBiolinkBlocks($legacyDb)
    {
        $this->info('Syncing biolink blocks (UPSERT)...');
        ...
        $this->line("  -> Biolink blocks: {$created} added, {$updated} updated.");
    }

    private function syncTrackLinks($legacyDb)
    {
        $this->info('Syncing track links (incremental append)...');
        
        $maxCurrentId = DB::table('track_links')->max('id') ?? 0;
        $totalNew = $legacyDb->table('track_links')->where('id', '>', $maxCurrentId)->count();

        if ($totalNew === 0) {
            $this->line("  -> Track links: Already up-to-date (0 new clicks).");
            return;
        }

        $bar = $this->output->createProgressBar($totalNew);
        $bar->start();

        $legacyDb->table('track_links')
            ->where('id', '>', $maxCurrentId)
            ->orderBy('id')
            ->chunk(1000, function ($clicks) use ($bar) {
                $insertData = [];
        ...
        $bar->finish();
        $this->newLine();
        $this->line("  -> Track links: {$totalNew} new click logs synchronized.");
    }
